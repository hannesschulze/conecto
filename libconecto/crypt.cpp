...
Crypt::generate_self_signed_cert (GnuTLSX509PrivateKey& key, const std::string& common_name)
{
    ...
    if (gnutls_x509_crt_init (&cert_raw) != 0) throw GnuTLSInitializationError ("Couldn't initialize certificate");
    ...
    std::list<DNSetting> dn_settings = { DNSetting (GNUTLS_OID_X520_ORGANIZATION_NAME, "conecto"),
    ...
    for (const auto& dn_val : dn_settings) {
        int err = gnutls_x509_crt_set_dn_by_oid (cert.get (), dn_val.oid.c_str (), 0, dn_val.name.c_str (),
                                                 dn_val.name.size ());
...
Crypt::generate_private_key ()
{
    ...
    if (gnutls_x509_privkey_init (&key_raw) != 0) throw GnuTLSInitializationError ("Couldn't initialize private key");
...
Crypt::export_to_file (const std::string& path, const std::string& data)
{
    try {
        ...
        std::string new_etag;
        file->replace_contents (data, "", new_etag, false,
                                Gio::FILE_CREATE_PRIVATE | Gio::FILE_CREATE_REPLACE_DESTINATION);
    } catch (Gio::Error& err) {
...
{
    gnutls_datum_t         datum { .data = (unsigned char*) certificate_pem.c_str (),
