    ...
    : Gtk::ListStore ()
{
    ...
    // Register signals for battery events
    m_battery_plugin = std::dynamic_pointer_cast<Conecto::Plugins::Battery>
        (Conecto::Backend::get_instance ().get_plugin ("kdeconnect.battery"));
    if (!m_battery_plugin) {
        m_battery_plugin = std::make_shared<Conecto::Plugins::Battery> ();
        Conecto::Backend::get_instance ().register_plugin (m_battery_plugin);
    }

    // Register signals for new devices
    m_connections.push_back (Conecto::Backend::get_instance ().signal_found_new_device ().connect
        (sigc::mem_fun (*this, &ConnectedDevices::on_new_device)));
    m_connections.push_back (m_battery_plugin->signal_battery ().connect
        (sigc::hide (sigc::hide (sigc::mem_fun (*this, &ConnectedDevices::on_new_device)))));
}
